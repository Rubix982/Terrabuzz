...
export const postLikeNotify = async(__id, __notificationSchemaForm) => {
    try {
        await API.postRequest(`${process.env.REACT_APP_API_URL}/notification-like`, __notificationSchemaForm);
        const { status } = await API.postRequest(`${process.env.REACT_APP_API_URL}/like/${__id}`);
        return status;        
    } catch(error) {
